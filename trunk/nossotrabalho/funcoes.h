...
string declarar_variaveis_temporarias()
{
	...
	{
		resultado += "int ";
		for(int i = 0; i < contador["int"].size(); i++)
		{
			resultado += "_int"+inttostr(i)+"=0"
			if(i+1 != contador["int"].size())
			{
				resultado += ","
			}
		}
	...
	{
		resultado += "int ";
		for(int i = 0; i < contador["bool"].size(); i++)
		{
			resultado += "_bool"+inttostr(i)
			if(i+1 != contador["bool"].size())
			{
				resultado += ","
			}
		}
	...
	{
		for(int i = 0; i < contador["char"].size(); i++)
		{
			resultado += "_char"+inttostr(i)
			if(i+1 != contador["char"].size())
			{
				resultado += ","
			}
		}
	...
	{
		for(int i = 0; i < contador["string"].size(); i++)
		{
			resultado += "_string"+inttostr(i)
			if(i+1 != contador["string"].size())
			{
				resultado += ","
			}		
		}
	...
	{
		for(int i = 0; i < contador["double"].size(); i++)
		{
			resultado += "_double"+inttostr(i)
			if(i+1 != contador["double"].size())
			{
				resultado += ","
			}		
		}
...
{
	n = n2;
	n.codigo = e + n2.valor + "_"  + inttosrc(nivel.back()) + ";\n";
}
...
void gera_vetor(string e, variavel &n, variavel n2, atributos_compilador n3)
{
	...
	n.ndim = 1;
	n.dim1 = n3.valor; 
	n.codigo = e + n2.valor + "_" + inttosrc(nivel.back()) + "[" + inttosrc(n3.valor) + "]; \n";
}
...
void gera_matriz(string e, variavel &n, variavel n2, atributos_compilador n3, atributos_compilador n4)
{
	...
	n.dim1 = n3.valor;
	n.dim2 = n4.valor;
	n.codigo = e + n2.valor + "_" + inttosrc(nivel.back()) + "[" + inttosrc(n5) + "]; \n";
}
...
void gera_FUN_ID(atributos_compilador &esquerda,atributos_compilador &primeiro,atributos_compilador &terceiro = NULL,atributos_compilador &quinto = NULL)
{
	if(terceiro == NULL && quinto == NULL)
	{
		sobe_nivel();
		esquerda.codigo = primeiro.codigo;
		return;	
	...
	if(terceiro != NULL & quinto == NULL)
	{
		...
		esquerda.codigo = primeiro.codigo;
		esquerda.n_dim = 1;
		esquerda.dim1 = strtoint(terceiro.valor); 
		return;	
	}
	if(terceiro != NULL & quinto != NULL)
	{
		...
		esquerda.n_dim = 2;
		esquerda.dim1 = strtoint(terceiro.valor); 
		esquerda.dim2 = strtoint(quinto.valor); 		
		return;	
	}
	
	yyerror("Erro no id da funcao");
}
...
void verifica_variavel(atributos_compilador &n)
{
	...
	{
		if(var[i].find(n) != var.at(i).end())
		{
			n.tipo = var.at(i)[str]->tipo;
			if(n.valor != "_ret")
				n.valor += "_" + int_to_str(nivel[i]);
			return;
		}
	}
	yyerror("Erro");
...
void gera_va(variavel &n, variavel n2)
{
...
void gera_va_vetor(variavel &n, variavel n2, atributos_compilador n3)
{
	verifica_variavel(n2);
	if(n2.tipo.ndim < 1) yyerror("Erro");
	if(n3.tipo.base != "int" || n3.tipo.ndim > 0) yyerror("Erro");
	string temp1, temp2;
	temp1 = transforma_variavel("bool");
	temp2 = label("ELSE");
	n.tipo = n2.tipo;
	n.codigo = n3.codigo + temp1 + " = " + int_to_str(n2.tipo.dim1) + " > " + n3.valor + ";\n";
	n.codigo += "if( " + temp1 + " )\ngoto " + temp2 + ";\ncout <<\"Index fora do alcance da matriz.\" << endl;\nexit(1);\n" + temp3 + ":\n";
	n.valor = n2.valor + "[" + n3.valor + "]";
	n.tipo.ndim--;
}

void gera_va_matriz(variavel &n, variavel n2, atributos_compilador n3, atributos_compilador n4)
{
	verifica_variavel(n2);
	if(n2.tipo.ndim < 2) yyerror("Erro");
	if(n4.tipo.base != "int" || n4.tipo.ndim > 0 || n3.tipo.base != "int" || n3.tipo.ndim > 0) yyerror("Erro");
	...
	n.tipo = n2.tipo;
	n.codigo = n4.codigo + n3.codigo + temp1 + " = " + int_to_str(n.tipo.dim1) + " > " + n4.valor + ";\n";
	n.codigo += temp2 + " = " + int_to_str(n4.tipo.dim2) + " > " + n3.valor + ";\n";
	n.codigo += temp3 + " = " + temp1 + " && " + temp2 + ";\n";
	n.codigo += temp4 + " = " + n4.valor + " * " + int_to_str(n2.tipo.dim2) + ";\n";
...
void gera_va2(int n, string n2, variavel &n3, variavel n4)
{
